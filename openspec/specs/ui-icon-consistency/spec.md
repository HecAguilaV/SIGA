# Spec: ui-icon-consistency

**Status**: Active
**Source change**: frontend-emoji-cleanup (archived 2026-06-20)
**Depends on**: ui-a2ui, dashboard-insights, sales-analytics (additive rule; functional behavior unchanged)
...
Critical/high indicators MUST use `WarningCircle` with host `color: var(--color-error)`; warning/medium MUST use `WarningCircle` with `color: var(--color-warning)`. Host MUST carry an aria-label naming the severity (e.g. `aria-label="Severidad: {severity}"`); SVG MUST be `aria-hidden="true"`.
...
Feature-card icons MUST use `weight="duotone"`; inline chip/badge indicators MUST use `weight="bold"`. The danger severity `WarningCircle` MAY use size 16 (design-table exception to the 14px chip/badge default). Decorative icons (adjacent `<h3>`, `<p class="kpi-title">`, or text labels the feature) MUST be `aria-hidden="true"`.
