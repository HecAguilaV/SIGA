...
## 1. Security Vision
SIGA operates under the **Zero-Trust** principle. No request is trusted by default, whether internal or external. Security is not a surface layer but the structural foundation of the distributed architecture.

## 2. Legal Framework and Compliance (Law 21.719)
This system is designed to comply with **Law 21.719 (Framework Law on Cybersecurity and Personal Data Protection)** of Chile. The system implements the following articles technically:
...
Isolation is **physical-logical**. Although the system shares the PostgreSQL database engine, each company (Tenant) operates in a vacuum:
